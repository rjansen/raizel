...
func (m *MockCloudSpanner) ExecuteStreamingSql(r *sppb.ExecuteSqlRequest, s sppb.Spanner_ExecuteStreamingSqlServer) error {
	...
	case "SELECT column1, column2, columnN from available":
		meta := KvMeta
		...
		err := s.Send(&sppb.PartialResultSet{
			Metadata: &meta,
			Values: []*proto3.Value{
				{Kind: &proto3.Value_NumberValue{NumberValue: 1}},
				{Kind: &proto3.Value_NumberValue{NumberValue: 2}},
				{Kind: &proto3.Value_StringValue{StringValue: "N"}},
			},
		})
		if err != nil {
			return err
